...
米国で先行しているサービスを週2回リサーチし、日本への落とし込み候補（2〜4社）を
...
| `setup_cron.sh` | cron ジョブ登録（手元のマシンで運用する場合のみ） |
...
GitHubのサーバー上で毎週火曜・金曜 8:00 JST に実行される。PCを起動しておく必要がない。
...
- カード枚数: その回に紹介する企業数（2〜4社）＋「レポート全文」カード1枚。
